...
function patchFile(filePath) {
  ...
  const isSameCheck = `const isSame = Array.isArray(roster.Size) ? ((roster.Size as any)[0]?.name === (roster.Population as any)[0]?.name && (roster.Size as any)[1]?.name === (roster.Population as any)[1]?.name) : ((roster.Size as any)?.name === (roster.Population as any)?.name);`;
