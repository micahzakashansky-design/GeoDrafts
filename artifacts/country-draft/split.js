...
modes.forEach(mode => {
  ...
  if (fs.existsSync(gameSrc)) {
    ...
    // Update imports in the game file
    let content = fs.readFileSync(gameDest, 'utf8');
    // Change ./GameShared to ./GameShared
    // Wait, the imports are already relative to `./`, so since they are in the same directory, 
    // imports like `./GameShared` or `./SidebarRoster` or `./SubmitDialog` stay the SAME!
    // But what about imports of components from other places? 
    // They use `@/lib` or `@/data` or `lucide-react`, so those stay the same!
    // This is incredibly easy because we're just grouping them.
    fs.writeFileSync(gameDest, content);
  }
...
modes.forEach(mode => {
  const oldImport = `import ${mode.gameFile.replace('.tsx', '')} from "@/pages/${mode.gameFile.replace('.tsx', '')}";`;
  const newImport = `import ${mode.gameFile.replace('.tsx', '')} from "@/pages/${mode.dir}/${mode.gameFile.replace('.tsx', '')}";`;
