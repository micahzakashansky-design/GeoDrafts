...
import type { PersonalLeaderboardEntry } from "@/lib/local-leaderboard";

import { formatAuthError } from "@/lib/auth-errors";

export function SubmitToGlobalDialog({ entry, mode, onClose, onSuccess }: { entry: PersonalLeaderboardEntry; mode: string; onClose: () => void; onSuccess: () => void; }) {
  ...
  async function handleGoogleSignIn() { setError(null); try { await signInWithGoogle(); } catch (e: any) { setError(formatAuthError(e)); } }
  async function handleEmailAuth() {
    setError(null);
    if (!email.trim() || !password) { setError(isSignUp ? "Enter your email and password." : "Enter your email or username and password."); return; }
    setLoading(true); try { await signInWithEmail(email.trim(), password, isSignUp); } catch (e: any) { setError(formatAuthError(e)); } finally { setLoading(false); }
  }
  async function handleSubmit() {
    if (!firebaseUser || !profile) return; setLoading(true); setError(null); try {
      await saveScore(firebaseUser.uid, profile.username, entry.score, mode, entry.roster || {}, entry.guesses, entry.mysteryCountry); 
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={onClose}>
      <motion.div initial={{ scale: 0.92, y: 16 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.92, y: 16 }} className="bg-background border border-border w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        {done ? (
          <div className="px-5 py-8 text-center"><div className="p-3 rounded-full bg-primary/10 w-fit mx-auto mb-4"><PartyPopper className="w-8 h-8 text-primary" /></div><div className="font-semibold text-foreground mb-1">Score submitted!</div><div className="text-sm text-muted-foreground mb-5"><span className="text-primary font-bold">{entry.score} pts</span> posted as <span className="font-semibold text-foreground">{displayName}</span>.</div><button onClick={onClose} className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-primary/20 text-primary border border-primary/40 font-semibold text-sm hover:bg-primary/30 transition-colors"><Trophy className="w-4 h-4" />Done</button></div>
        ) : authLoading ? ( <div className="px-5 py-8 text-center text-sm text-muted-foreground animate-pulse">Checking account…</div>
        ) : !firebaseUser || firebaseUser.isAnonymous || !profile ? (
          <div className="px-5 py-5 space-y-3">
            <div className="text-center mb-2"><div className="p-3 rounded-full bg-foreground/10 w-fit mx-auto mb-3"><Lock className="w-6 h-6 text-muted-foreground" /></div><div className="font-semibold text-foreground">Sign in to submit</div><p className="text-xs text-muted-foreground mt-1">Post your <span className="text-primary font-bold">{entry.score} pts</span> · {modeLabel} to the global leaderboard.</p></div>
            ...
            ) : ( <div className="space-y-2"><input type="text" autoCapitalize="none" autoCorrect="off" spellCheck={false} value={email} onChange={e => setEmail(e.target.value)} placeholder={isSignUp ? "Email address" : "Email or Username"} className="w-full px-3 py-2 rounded-lg border border-border bg-foreground/5 text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/50 text-sm" /><input type="password" value={password} onChange={e => setPassword(e.target.value)} onKeyDown={e => e.key === "Enter" && handleEmailAuth()} placeholder="Password" className="w-full px-3 py-2 rounded-lg border border-border bg-foreground/5 text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/50 text-sm" />{error && <p className="text-xs text-red-400 font-medium">{error}</p>}<button onClick={handleEmailAuth} disabled={loading} className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-primary/20 text-primary border border-primary/40 font-semibold text-sm hover:bg-primary/30 transition-colors disabled:opacity-50"><Send className="w-4 h-4" />{loading ? "…" : isSignUp ? "Create Account" : "Sign In"}</button><div className="flex items-center justify-between text-xs"><button onClick={() => setShowEmail(false)} className="text-muted-foreground hover:text-foreground">← Back</button><button onClick={() => { setIsSignUp(!isSignUp); setError(null); }} className="text-primary hover:underline">{isSignUp ? "Have an account? Sign in" : "New here? Create account"}</button></div></div> )}
          </div>
          ...
          <div className="px-5 py-4 space-y-4">
            <div className="flex items-center justify-between text-sm"><span className="text-muted-foreground">Your score</span><span className="font-bold text-primary">{entry.score} pts · {modeLabel}</span></div>
