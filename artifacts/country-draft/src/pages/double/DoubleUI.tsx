...
export function SelectionPhase({ options, onPick, isHardMode, mode, targetCategory }: { options: Country[]; onPick: (c: Country) => void; isHardMode: boolean; mode: string; targetCategory?: Category | null; }) {
  return (
    <div className="p-6 flex flex-col gap-6 items-center justify-start flex-1 overflow-y-auto w-full">
      <div className="text-center"><h2 className="text-3xl font-sans font-bold mb-1 text-foreground">{targetCategory ? "Wildcard Replacement" : mode === "sabotage" ? "Sabotage Choice" : "Double Draft Choice"}</h2><p className="text-muted-foreground text-sm">{targetCategory ? `Choose a country to replace your ${targetCategory} slot.` : mode === "sabotage" ? "Pick a country for your opponent to use." : "Choose which country to add to your roster."}</p></div>
...
export function GuessPhase({ mysteryCountry, guesses, onGuess }: { mysteryCountry: Country; guesses: string[]; onGuess: (name: string) => void; }) {
  ...
  return (
    <div className="p-6 flex flex-col gap-6 items-center justify-center flex-1 max-w-5xl mx-auto w-full">
      <div className="text-center"><h2 className="text-4xl font-sans font-bold mb-2 text-foreground">Guess the Country</h2><p className="text-muted-foreground text-sm max-w-md mx-auto">Use the numeric ratings below to identify the mystery nation. Be precise!</p></div>
      <div className="w-full max-w-md relative mt-4 mb-6">
        <div className="relative group"><div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none"><Search className="w-5 h-5 text-muted-foreground group-focus-within:text-primary transition-colors" /></div>
          <input type="text" value={input} onChange={e => { setInput(e.target.value); setShowSuggestions(true); }} onKeyDown={e => { if (e.key === "Enter" && input.trim() === "bypass:devtest3781") { onGuess(input.trim()); setInput(""); setShowSuggestions(false); } }} placeholder="Start typing a country name..." className="w-full bg-card border border-border rounded-2xl pl-12 pr-4 py-4 text-lg focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all shadow-inner" onFocus={() => setShowSuggestions(true)} />
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 w-full">{categories.map(cat => { const key = getCategoryKey(cat); const score = stats[key].score; return (<div key={cat} className="p-4 rounded-xl border border-border bg-card flex flex-col items-center text-center shadow-sm"><div className="text-primary/70 mb-1.5">{CATEGORY_ICONS[cat]}</div><div className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider mb-1">{cat}</div><div className="text-3xl font-bold text-foreground tracking-tighter">{score}</div></div>); })}</div>
    </div>
  );
}
      ...
      <div className="max-w-4xl mx-auto space-y-6 md:space-y-8 pb-20">
        ...
        <div className={`grid grid-cols-1 sm:grid-cols-2 ${bPath ? "lg:grid-cols-3" : ""} gap-4 animate-in slide-in-from-bottom-8 fade-in duration-700 delay-200`}>
          <div className="p-4 md:p-5 rounded-2xl bg-card border border-border shadow-sm flex items-start gap-4">
            <div className="p-3 bg-muted rounded-xl shrink-0">{archetype.icon}</div>
          ...
          <div className="p-4 md:p-5 rounded-2xl bg-card border border-border shadow-sm flex items-start gap-4">
            <div className="p-3 bg-muted rounded-xl shrink-0">{rating.icon}</div>
          ...
          {bPath && (
            <div className="p-4 md:p-5 rounded-2xl bg-card border border-border shadow-sm flex items-start gap-4">
              <div className="p-3 bg-muted rounded-xl shrink-0">{bPath === "agricultural" ? <Leaf className="w-5 h-5 text-yellow-400" /> : bPath === "extraction" ? <Mountain className="w-5 h-5 text-yellow-400" /> : <Building className="w-5 h-5 text-yellow-400" />}</div>
        ...
        <div className="space-y-4">
          <h3 className="text-lg md:text-xl font-sans font-bold text-foreground px-2 flex items-center justify-between">
            ...
            {wildcardPhase && (
              <button onClick={() => setWildcardPhase(false)} className="font-sans text-xs flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-muted text-muted-foreground font-semibold hover:text-foreground transition-colors">
                Cancel
              </button>
            )}
          </h3>
                  ...
                  return (
                    <div key={cat} className="grid grid-cols-2 gap-2 md:gap-3 p-0 border-0 bg-transparent">
                      {["Size", "Population"].map(splitCat => {
                        ...
                        return (
                          <div key={sCat} onClick={() => onWildcardSelect(sCat)} className="p-3 md:p-4 rounded-2xl border flex flex-col gap-2 relative transition-all cursor-pointer hover:border-blue-500/50 hover:bg-blue-500/5 hover:scale-[1.02] border-border bg-card">
                            ...
                            {!isHardMode && (
                              <div className="flex flex-col items-start gap-1 mt-3">
                                <div className="flex items-center gap-2 flex-wrap">
                                  ...
                                  <div className="text-[8px] font-bold px-1.5 py-0.5 rounded w-max text-foreground bg-muted">Bonus</div>
                                </div>
                              </div>
                            )}
                              ...
                              <div className="space-y-1 flex flex-col items-center">
                                {!isHardMode && (
                                  <div className="flex flex-col gap-1 items-center">
                                    ...
                                    <div className="text-[8px] font-bold px-1.5 py-0.5 rounded w-max text-foreground bg-muted">Size Bonus</div>
                                  </div>
                                )}
                                <p className="text-[10px] md:text-xs text-muted-foreground leading-snug italic line-clamp-3 mt-1 text-center">"{(sizeCountry as any).stats.size.description}"</p>
                              </div>
                            </div>
                              ...
                              <div className="space-y-1 flex flex-col items-center">
                                {!isHardMode && (
                                  <div className="flex flex-col gap-1 items-center">
                                    ...
                                    <div className="text-[8px] font-bold px-1.5 py-0.5 rounded w-max text-foreground bg-muted">Pop Bonus</div>
                                  </div>
                                )}
                                <p className="text-[10px] md:text-xs text-muted-foreground leading-snug italic line-clamp-3 mt-1 text-center">"{(popCountry as any).stats.population.description}"</p>
                              </div>
                            </div>
                          </div>
                        ) : (
