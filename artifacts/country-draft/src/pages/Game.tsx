...
function pngScoreBar(score: number): string {
  return (
    PNG_COLORS.scoreColors[Math.max(0, Math.min(10, score)) - 1] ?? "#eab308"
  );
}
...
async function drawRosterPng(
  ...
  displayCats.forEach((category, i) => {
    ...
    if (country) {
      ...
      if (score !== null) {
        ...
        ctx.fillText(
          `${pngScoreLabel(score, weight)} · ${pts} pts`,
          cx + 46,
          cy + 80,
        );
        ...
        let trunc = desc;
        while (ctx.measureText(trunc).width > maxDescW && trunc.length > 0)
        ...
        ctx.fillStyle = C.gold;
        ctx.font = "bold 11px sans-serif";
        ctx.fillText("Bonus Contributor", cx + 46, cy + 70);
      }
    } else {
      ctx.fillStyle = C.muted;
  ...
  canvas.toBlob((blob) => {
    ...
    a.href = url;
    a.download = "geodrafts-nation.png";
    a.click();
...
const CATEGORY_WEIGHTS: Partial<Record<Category, number>> = {
  ...
  Economy: 1.5,
  Government: 1.5,
  "International Relationships": 1.2,
  Technology: 1.2,
  Education: 1.2,
...
function getCategoryStars(cat: Category): string {
  if (cat === "Military" || cat === "Economy" || cat === "Government")
    return "★★★";
  if (
    cat === "International Relationships" ||
    cat === "Technology" ||
    cat === "Education" ||
    cat === "Natural Resources" ||
    cat === "Healthcare" ||
    cat === "Size" ||
    cat === "Population"
  )
...
function ExpandableDescription({ description }: { description: string }) {
  ...
  useEffect(() => {
    ...
    if (el) setIsTruncated(el.scrollHeight > el.offsetHeight);
  }, [description]);
  return (
...
function getStateKey(isDailyMode: boolean, dailyDate: string): string {
  return isDailyMode
    ? `countryDraftState_daily_${dailyDate}`
    : "countryDraftState_v4";
}
...
function freshGame(
  ...
  const mode = (forcedMode ||
    localStorage.getItem("countryDraftMode") ||
    "normal") as GameState["mode"];
  const isHardMode =
    forcedHardMode ?? localStorage.getItem("countryDraftHardMode") === "true";
  const effectiveSeed =
    seed ??
    (isDailyMode
      ? dateStrToSeed(dailyDate)
      : Math.floor(Math.random() * 1000000));
  const pool = seededShuffle([...COUNTRIES], effectiveSeed);
  let selectionOptions: Country[] | null = null;
  let currentCountry: Country | null = null;
  let mysteryCountry: Country | null = null;
  let remainingPool = pool;
  if (mode === "double" || mode === "sabotage") {
    selectionOptions = [pool[0], pool[1]];
    remainingPool = pool.slice(2);
  } else if (mode === "guess") {
    mysteryCountry = pool[0];
...
function saveGameState(state: GameState) {
  localStorage.setItem(
    getStateKey(state.isDailyMode, state.dailyDate),
    JSON.stringify(state),
  );
}
function saveDailyResult(dailyDate: string, score: number) {
  localStorage.setItem(
    `countryDraftDailyResult_${dailyDate}`,
    JSON.stringify({ score, completed: true }),
  );
}
function loadLocalLeaderboard(): LocalLeaderboardEntry[] {
  try {
    return JSON.parse(
      localStorage.getItem("countryDraftLeaderboard_v2") || "[]",
    );
  } catch {
    return [];
  }
...
function getRating(total: number): {
  ...
  return {
    label: "Struggling State",
    color: "text-red-400",
    icon: <Shield className="w-5 h-5" />,
  };
}
function computeSizePopBonus(
  roster: Partial<Record<Category, Country>>,
): number {
  ...
  if (idealDiff < -2) {
    fitMultiplier = Math.max(0, 1.0 - (-idealDiff - 2) * 0.4);
  } else if (idealDiff > 6) {
...
function getCountryArchetype(
  roster: Partial<Record<Category, Country>>,
): Archetype {
  const s = (cat: Category): number =>
    BONUS_CATEGORIES.includes(cat)
      ? 0
      : (roster[cat]?.stats[getCategoryKey(cat)].score ?? 0);
  const mil = s("Military"),
    eco = s("Economy"),
    gov = s("Government"),
    intl = s("International Relationships"),
    tech = s("Technology"),
    cult = s("Culture"),
    health = s("Healthcare"),
    climate = s("Climate"),
  ...
  if (mil >= 9 && eco >= 8)
    return {
      ...
      description:
        "A dominant force projecting military might backed by formidable economic power.",
    };
  if (mil >= 9)
    return {
      ...
      description:
        "An imposing military force built for security and strategic dominance.",
    };
  if (eco >= 9 && gov >= 8)
    return {
      ...
      description:
        "A well-governed nation driving global economic growth and innovation.",
    };
  if (edu >= 9 && tech >= 8)
    return {
      ...
      description:
        "Innovation powered by world-class education and cutting-edge technology.",
    };
  if (gov >= 9 && health >= 8)
    return {
      ...
      description:
        "A model of governance — transparent, effective, and caring for its citizens.",
    };
  if (tech >= 9 && eco >= 7)
    return {
      title: "Tech Nation",
      ...
      description:
        "A forward-thinking nation built on innovation and digital infrastructure.",
    };
  if (nat >= 9 && eco >= 7)
    return {
      title: "Resource Giant",
      icon: <Mountain className="w-5 h-5 text-orange-400" />,
      description:
        "A nation whose natural wealth fuels economic dominance and global leverage.",
    };
  if (intl >= 9 && loc >= 7)
    return {
      title: "Diplomatic Powerhouse",
      icon: <GlobeIcon className="w-5 h-5 text-primary" />,
      description:
        "Strategically positioned and globally connected — allies everywhere.",
    };
  if (intl >= 9)
    return {
      ...
      description:
        "A skilled navigator of global politics with allies on every continent.",
    };
  if (climate >= 9 && tour >= 8)
    return {
      title: "Tourist Paradise",
      icon: <Umbrella className="w-5 h-5 text-yellow-400" />,
      description:
        "Blessed with breathtaking scenery and a world-famous tourism destination.",
    };
  if (hist >= 9 && cult >= 8)
    return {
      title: "Cultural Empire",
      icon: <Palette className="w-5 h-5 text-purple-400" />,
      description:
        "A civilization with deep cultural roots and an enduring historical legacy.",
    };
  if (health >= 9 && edu >= 8)
    return {
      title: "Welfare State",
      ...
      description:
        "A nation that puts its people first with world-class healthcare and education.",
    };
  if (cult >= 9)
    return {
      title: "Cultural Giant",
      icon: <Palette className="w-5 h-5 text-purple-400" />,
      description:
        "A vibrant nation with rich traditions, arts, and cultural influence worldwide.",
    };
  if (tour >= 9)
    return {
      ...
      description:
        "A destination the world dreams of visiting, with iconic landscapes and attractions.",
    };
  if (climate >= 9)
    return {
      title: "Natural Paradise",
      icon: <Leaf className="w-5 h-5 text-green-400" />,
      description: "Blessed with exceptional climate and natural beauty.",
    };
  if (nat >= 8)
    return {
      title: "Resource Rich",
      icon: <Mountain className="w-5 h-5 text-orange-400" />,
      description:
        "Blessed with abundant natural resources powering growth and exports.",
    };
  const values = [
    mil,
    eco,
  ...
  if (avg >= 7.5)
    return {
      title: "World Power",
      icon: <GlobeIcon className="w-5 h-5 text-primary" />,
      description: "A well-rounded nation excelling across multiple domains.",
    };
  if (avg >= 5.5)
    return {
      title: "Emerging Nation",
      icon: <TrendingUp className="w-5 h-5 text-emerald-400" />,
      description:
        "A country with clear strengths building toward global relevance.",
    };
  return {
    title: "Developing State",
    icon: <Leaf className="w-5 h-5 text-green-400" />,
    description: "A nation still finding its footing on the world stage.",
  };
}
...
function LocalLeaderboardRow({
  ...
  return (
    <div
      className={`rounded-lg border overflow-hidden transition-colors ${isCurrentGame ? "border-primary/60 bg-primary/5" : "border-border/60 bg-card/30"}`}
    >
      <button
      ...
      >
        ...
        {isCurrentGame && (
          <span className="text-xs bg-primary/20 text-primary px-1.5 py-0.5 rounded font-semibold mr-1">
            This game
          </span>
        )}
        <ChevronDown
        ...
        {done ? (
          <div className="px-5 py-8 text-center">
            ...
            <div className="text-sm text-muted-foreground mb-5">
              <span className="text-primary font-bold">{score} pts</span> posted
              as{" "}
          ...
          <div className="px-5 py-5 space-y-3">
            <div className="text-center mb-2">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                ...
                {modeLabel} to the global leaderboard.
              </p>
            </div>
            {!showEmail ? (
              <>
                ...
                >
                  <svg className="w-4 h-4" viewBox="0 0 48 48">
                    <path
                      fill="#EA4335"
                      d="M24 9.5c3.5 0 6.5 1.2 8.9 3.2l6.6-6.6C35.5 2.5 30.1 0 24 0 14.7 0 6.7 5.4 2.7 13.3l7.7 6C12.2 13.3 17.7 9.5 24 9.5z"
                    />
                    <path
                      fill="#4285F4"
                      d="M46.1 24.6c0-1.5-.1-3-.4-4.4H24v8.4h12.4c-.5 2.8-2.1 5.2-4.5 6.8l7 5.4c4.1-3.8 6.5-9.4 6.5-16.2z"
                    />
                    <path
                      fill="#FBBC05"
                      d="M10.4 28.7A14.6 14.6 0 0 1 9.5 24c0-1.6.3-3.2.8-4.7l-7.7-6A24 24 0 0 0 0 24c0 3.9.9 7.5 2.7 10.7l7.7-6z"
                    />
                    <path
                      fill="#34A853"
                      d="M24 48c6.1 0 11.3-2 15-5.5l-7-5.4c-2 1.3-4.5 2.1-8 2.1-6.3 0-11.7-4.3-13.6-10l-7.7 6C6.7 42.6 14.7 48 24 48z"
                    />
                  </svg>
              ...
              <div className="space-y-2">
                ...
                <button
                  ...
                  disabled={loading}
                  className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-primary/20 text-primary border border-primary/40 font-semibold text-sm hover:bg-primary/30 transition-colors disabled:opacity-50"
                >
          ...
          <div className="px-5 py-4 space-y-4">
            ...
            <button
              ...
              disabled={loading}
              className={`w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg font-semibold text-sm transition-colors ${!loading ? "bg-primary/20 text-primary border border-primary/40 hover:bg-primary/30" : "bg-secondary text-muted-foreground border border-border cursor-not-allowed"}`}
            >
              <Send className="w-4 h-4" />
    ...
    <div className="p-6 flex flex-col gap-6 items-center justify-center flex-1">
      <div className="text-center">
        <h2 className="text-3xl font-serif font-bold mb-1">
          {mode === "sabotage" ? "Sabotage Choice" : "Double Draft Choice"}
        </h2>
        <p className="text-muted-foreground text-sm">
          {mode === "sabotage"
            ? "Pick a country for your opponent to use."
            : "Choose which country to add to your roster."}
        </p>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-4xl">
        {options.map((country, idx) => (
          <motion.button
            ...
            className="bg-card border border-border rounded-2xl overflow-hidden shadow-xl text-left group"
          >
            <div className="p-8 border-b border-border bg-secondary/10 flex flex-col items-center text-center">
            ...
            <div className="p-5 space-y-4">
              <p className="text-sm text-foreground/70 leading-relaxed italic line-clamp-2">
                "{country.knownFor}"
              </p>
              {!isHardMode && (
                <div className="grid grid-cols-3 gap-2">
                  {["Military", "Economy", "Government"].map((cat) => {
                    const score =
                      country.stats[getCategoryKey(cat as Category)].score;
                    return (
                      <div
                        key={cat}
                        className="text-center p-2 rounded-lg bg-secondary/30 border border-border/40"
                      >
              ...
              <div className="w-full py-2.5 rounded-xl bg-primary/10 text-primary text-center font-bold text-sm group-hover:bg-primary group-hover:text-primary-foreground transition-colors border border-primary/20">
                {mode === "sabotage"
                  ? `Give ${country.name}`
                  : `Pick ${country.name}`}
              </div>
    ...
    <div className="p-6 flex flex-col gap-6 items-center justify-center flex-1 max-w-5xl mx-auto w-full">
      <div className="text-center">
        ...
        <p className="text-muted-foreground text-sm max-w-md mx-auto">
          Use the numeric ratings below to identify the mystery nation. Be
          precise!
        </p>
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 w-full">
        {categories.map((cat) => {
          ...
          return (
            <div
              key={cat}
              className="p-4 rounded-xl border border-border bg-card/40 flex flex-col items-center text-center shadow-sm"
            >
              <div className="text-primary/70 mb-1.5">
                {CATEGORY_ICONS[cat]}
              </div>
      ...
      <div className="w-full max-w-md relative mt-4">
        <div className="relative group">
          ...
          <input
            ...
            className="w-full bg-secondary/50 border border-border rounded-2xl pl-12 pr-4 py-4 text-lg focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all shadow-inner"
          ...
          {showSuggestions && suggestions.length > 0 && (
            <motion.div
              ...
              className="absolute bottom-full left-0 w-full mb-3 bg-card border border-border rounded-2xl shadow-2xl overflow-hidden z-50"
            >
              {suggestions.map((s) => (
                <button
                  key={s.name}
        ...
        {guesses.length > 0 && (
          <div className="mt-8 space-y-3">
            <div className="flex items-center justify-center gap-2 text-xs font-bold text-muted-foreground uppercase tracking-widest">
              ...
              Attempt History ({guesses.length})
            </div>
            <div className="flex flex-wrap gap-2 justify-center">
              {guesses.map((g, i) => (
                <motion.div
                  ...
                  className="px-4 py-1.5 rounded-full bg-red-500/10 border border-red-500/20 text-red-400 text-sm font-bold shadow-sm"
                >
                  {g}
                </motion.div>
              ))}
      ...
      >
        ...
        <div className="bg-primary/5 border border-primary/20 rounded-lg px-4 py-2.5 flex items-center gap-3">
          ...
          <p className="text-sm text-foreground/80">
            {isHardMode ? (
              <>
                <span className="text-red-400 font-semibold">Hard Mode</span> —
                no ratings.{" "}
                <span className="text-primary font-semibold">Click a slot</span>{" "}
                to assign based on objective stats.
              </>
            ) : (
              <>
                <span className="text-primary font-semibold">Hover a slot</span>{" "}
                on the left to preview this country's score, then click to
                assign.
              </>
            )}
          </p>
...
function GameOver({
  ...
  setWildcardPhase: (val: boolean) => void;
  ...
  return (
    <div className="p-6 flex flex-col gap-8 max-w-5xl mx-auto w-full">
      <motion.div
        ...
        animate={{ opacity: 1, y: 0 }}
        className="bg-card/50 border border-border p-8 rounded-3xl text-center shadow-2xl relative overflow-hidden"
      >
        ...
        <div className="relative">
          ...
          <h2 className="font-serif text-5xl font-bold mb-8 text-foreground tracking-tight">
            Your Ideal Nation
          </h2>
          <div className="flex items-center justify-center gap-12 flex-wrap mb-8">
            ...
            <div className="text-center group">
              <div
                className={`text-3xl font-bold ${rating.color} flex items-center justify-center gap-2`}
              >
      ...
      {room && room.status === "finished" && (
        <motion.div
          ...
          className="bg-primary/5 border border-primary/20 p-8 rounded-3xl shadow-xl"
        >
          <div className="flex items-center gap-3 mb-6">
            <Trophy className="w-6 h-6 text-yellow-400" />
            <h3 className="text-2xl font-serif font-bold">Final Results</h3>
          </div>
          <div className="space-y-3">
            {sortedPlayers.map((p, i) => (
              <div
                key={p.uid}
                className={`flex items-center justify-between p-5 rounded-2xl border ${i === 0 ? "bg-yellow-400/10 border-yellow-400/30 scale-[1.02] shadow-lg" : "bg-card border-border"}`}
              >
                <div className="flex items-center gap-4">
                  <div
                    className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold text-lg ${i === 0 ? "bg-yellow-400 text-yellow-950" : i === 1 ? "bg-slate-300 text-slate-900" : i === 2 ? "bg-amber-600 text-amber-950" : "bg-secondary text-muted-foreground"}`}
                  >
                    {i + 1}
                  </div>
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground">
                      Draft Rank: {pngRatingLabel(p.score)}
                    </div>
                  </div>
                </div>
      ...
      <div className="flex justify-center gap-4 flex-wrap">
        {!isDailyMode && !room && (
          <button
            ...
            className="flex items-center gap-2 px-6 py-3 bg-yellow-400/20 text-yellow-400 border border-yellow-400/40 rounded-xl font-bold text-sm hover:bg-yellow-400/30 transition-all shadow-lg hover:scale-105"
          >
        ...
        <button
          onClick={onDownload}
          className="flex items-center gap-2 px-6 py-3 bg-primary/20 text-primary border border-primary/40 rounded-xl font-bold text-sm hover:bg-primary/30 transition-all shadow-lg hover:scale-105"
        >
          <Download className="w-4 h-4" /> Download Nation PNG
        </button>
        {gameMode === "easy" &&
          !wildcardUsed &&
          !wildcardPhase &&
          ...
          !room && (
            <button
              onClick={onWildcard}
              className="flex items-center gap-2 px-6 py-3 bg-blue-600/20 text-blue-300 border border-blue-500/40 rounded-xl font-bold text-sm hover:bg-blue-600/30 transition-all shadow-lg hover:scale-105"
            >
              <Shuffle className="w-4 h-4" /> Activate Wildcard
            </button>
          )}
        <button
          onClick={onReset}
          className="flex items-center gap-2 px-6 py-3 bg-secondary text-foreground rounded-xl font-bold text-sm hover:bg-secondary/70 transition-all border border-border shadow-md hover:scale-105"
        >
          <RotateCcw className="w-4 h-4" />{" "}
          {isDailyMode || room ? "Return Home" : "Start New Draft"}
        </button>
      </div>
      <div
        ref={rosterRef}
        className="rounded-3xl overflow-hidden border border-border shadow-2xl bg-card/20 backdrop-blur-sm"
      >
        {wildcardPhase && (
          <div className="bg-blue-500/10 border-b border-blue-500/30 px-8 py-4 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <Shuffle className="w-6 h-6 text-blue-400 animate-spin-slow" />
              <div>
                <div className="text-lg font-bold text-blue-300">
                  Wildcard Active
                </div>
                <div className="text-xs text-blue-400/70">
                  Pick a slot to swap it for a mystery country.
                </div>
              </div>
            </div>
            <button
              onClick={() => setWildcardPhase(false)}
              className="p-2 rounded-xl text-blue-400/60 hover:text-blue-300 hover:bg-blue-500/20 transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
      ...
      <div className="rounded-3xl border border-border overflow-hidden bg-card/20 shadow-xl">
        <div className="bg-secondary/30 px-6 py-4 border-b border-border flex items-center justify-between">
          ...
          <span className="text-[10px] font-medium text-muted-foreground">
            CLICK ROW TO VIEW ROSTER
          </span>
        </div>
        {leaderboard.length === 0 ? (
          <div className="p-12 text-center text-sm text-muted-foreground/40 italic">
            Your best drafts will appear here.
          </div>
        ) : (
...
export default function Game() {
  ...
  const dailyDate =
    localStorage.getItem("countryDraftDailyDate") || getTodayStr();
  const roomCode = localStorage.getItem("countryDraftRoomCode");
  const forcedMode = localStorage.getItem("countryDraftMode");
  const [state, setState] = useState<GameState>(() => {
    const loaded = loadGameState(isDailyMode, dailyDate);
  ...
  const [isHardMode, setIsHardMode] = useState<boolean>(state.isHardMode);
  ...
  useEffect(() => {
    ...
    if (isDailyMode && firebaseUser) {
      saveDailyState(firebaseUser.uid, dailyDate, {
        ...state,
        totalScore,
      }).catch(() => {});
    }
  }, [state, isDailyMode, dailyDate, firebaseUser, totalScore]);
  useEffect(() => {
    if (!roomCode || !firebaseUser) return;
    const unsubRoom = listenToRoom(roomCode, (r) => {
      setRoom(r);
      if (r.status === "finished" && !state.gameOver) {
  ...
  }, [roomCode, firebaseUser, state.gameOver]);
  useEffect(() => {
    if (room && room.poolSeed !== state.poolSeed) {
      setState(
        freshGame(
          false,
          "",
  ...
  useEffect(() => {
    if (roomCode && firebaseUser && room && room.status === "playing") {
      const myPlayer = players.find((p) => p.uid === firebaseUser.uid);
      if (!myPlayer) return;
      const roomRound = room.currentRound;
      const localRound = Object.keys(state.roster).length;
      if (roomRound > localRound) {
        const poolIdx =
          state.mode === "sabotage" || state.mode === "double"
            ? roomRound * 2
            : roomRound;
        const nextOptions = state.pool.slice(poolIdx, poolIdx + 2);
        const nextCountry =
          state.mode === "party" || state.mode === "normal"
            ? state.pool[poolIdx]
            : null;
        setState((prev) => ({
          ...prev,
          ...
          guesses: state.mode === "guess" ? [] : prev.guesses,
        }));
        updatePlayer(roomCode, firebaseUser.uid, {
          finishedRound: false,
          sabotageChoice: null,
        });
      }
      if (
        ...
        if (otherPlayer && otherPlayer.sabotageChoice) {
          const choice = COUNTRIES.find(
            (c) => c.name === otherPlayer.sabotageChoice,
          );
          if (choice) {
            setState((prev) => ({ ...prev, currentCountry: choice }));
          }
        }
      }
      const allFinished =
        players.length > 0 && players.every((p) => p.finishedRound);
      if (
        allFinished &&
        firebaseUser.uid === room.hostId &&
        room.currentRound === localRound
      ) {
        if (room.currentRound < CATEGORIES.length - 1) {
          updateRoom(roomCode, { currentRound: room.currentRound + 1 });
        } else {
  ...
  const onGuess = useCallback(
    (name: string) => {
      ...
      const correct =
        name.toLowerCase() === state.mysteryCountry.name.toLowerCase();
      if (correct) {
        setState((prev) => ({
          ...prev,
          currentCountry: state.mysteryCountry,
          mysteryCountry: null,
          guesses: [...prev.guesses, name],
        }));
      } else {
        setState((prev) => ({ ...prev, guesses: [...prev.guesses, name] }));
      }
    },
  ...
  const assignCountry = useCallback(
    (category: Category) => {
      if (!state.currentCountry || state.roster[category] || state.gameOver)
        return;
      ...
      let nextOptions: Country[] | null = null;
      let nextCountry: Country | null = null;
      if (!allFilled && !roomCode) {
        if (state.mode === "double" || state.mode === "sabotage") {
          const startIdx = Object.keys(newRoster).length * 2;
          nextOptions = state.pool.slice(startIdx, startIdx + 2);
        } else {
          const startIdx = Object.keys(newRoster).length;
          nextCountry = state.pool[startIdx];
        }
      }
      ...
      if (roomCode && firebaseUser) {
        updatePlayer(roomCode, firebaseUser.uid, {
          roster: Object.fromEntries(
            Object.entries(newRoster).map(([cat, c]) => [cat, c.name]),
          ),
          score: computeTotalScore(newRoster),
          finishedRound: true,
        });
      }
      setState((prev) => ({
  ...
  const doReset = useCallback(() => {
    if (isDailyMode || roomCode) {
      ...
      localStorage.removeItem("countryDraftMode");
      navigate("/");
      return;
    }
  ...
  }, [isDailyMode, roomCode, navigate]);
  const toggleHardMode = useCallback(() => {
    if (isDailyMode || roomCode) return;
    const next = !isHardMode;
    setIsHardMode(next);
    localStorage.setItem("countryDraftHardMode", String(next));
    localStorage.removeItem("countryDraftState_v4");
    setState(freshGame(false, ""));
  }, [isHardMode, isDailyMode, roomCode]);
  const startWildcard = useCallback(() => {
    if (state.mode !== "normal" || state.wildcardUsed || !state.gameOver)
      return;
  ...
  const applyWildcard = useCallback(
    (category: Category) => {
      if (
        ...
        return;
      const newCountry = state.pool[state.pool.length - 1];
      const newRoster = { ...state.roster, [category]: newCountry };
      setWildcardPhase(false);
      setLastAssigned(category);
      setTimeout(() => setLastAssigned(null), 600);
      setState((prev) => ({ ...prev, roster: newRoster, wildcardUsed: true }));
    },
    [state, wildcardPhase],
  );
    ...
    >
      <header className="border-b border-border px-4 py-2.5 flex items-center justify-between bg-card/50 backdrop-blur-sm sticky top-0 z-40">
        <div className="flex items-center gap-2">
          <button
            onClick={() => {
              if (state.gameOver || Object.keys(state.roster).length === 0) {
                navigate("/");
              } else {
          ...
          {isDailyMode ? (
            ...
            <span className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-blue-500/15 border border-blue-500/30 text-blue-400 text-xs font-bold">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={toggleTheme}
            className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-xs text-muted-foreground hover:text-foreground hover:bg-secondary border border-border transition-colors"
          >
            {isLight ? (
        ...
        <div className="flex-1 flex flex-col overflow-y-auto relative">
          {state.gameOver ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
              ...
              <h2 className="text-3xl font-serif font-bold mb-2">Game Lobby</h2>
              ...
              {firebaseUser?.uid === room.hostId ? (
                <button
                  ...
                  className="px-12 py-4 rounded-2xl bg-primary text-primary-foreground font-bold text-lg hover:opacity-90 transition-opacity disabled:opacity-50 shadow-xl"
                >
                  Start Match
                </button>
              ) : (
            ...
            room.status === "playing" &&
            players.find((p) => p.uid === firebaseUser?.uid)?.finishedRound &&
            !players.every((p) => p.finishedRound) ? (
            <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
              ...
              <p className="text-muted-foreground">
                The next country will be revealed once everyone finishes this
                round.
              </p>
            </div>
            ...
            <SelectionPhase
              ...
              onPick={(c) => {
                if (state.mode === "sabotage" && roomCode && firebaseUser) {
                  updatePlayer(roomCode, firebaseUser.uid, {
                    sabotageChoice: c.name,
                  });
            ...
            <div className="flex-1 flex items-center justify-center text-muted-foreground">
              <div className="text-center">
                ...
                <p>Waiting for game to advance...</p>
              </div>
            </div>
          )}
        </div>
      ...
      <AnimatePresence>
        {showSubmitDialog && (
          <SubmitDialog
            ...
            onSuccess={() =>
              setState((prev) => ({ ...prev, leaderboardSubmitted: true }))
            }
          />
        )}
      </AnimatePresence>
