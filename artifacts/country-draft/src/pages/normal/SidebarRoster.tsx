...
export function SidebarRoster({ roster, isHardMode , categoryTimes}: { roster: Partial<Record<Category, Country>>; isHardMode?: boolean ; categoryTimes?: Partial<Record<Category, number>>; }) {
  ...
  return (
    <div className="flex flex-col h-full min-h-[400px]">
      <div className="border-b border-border pb-2 mb-3 px-1 shrink-0">
      ...
      <div className="space-y-1.5 flex-1 overflow-y-auto">
        {Object.keys(roster).length === 0 ? (
          <div className="flex flex-col items-center justify-center text-center h-full pb-12 mt-12">
            ...
            <p className="text-sm font-semibold text-foreground mb-1">Your roster is empty</p>
        ...
        {hasSizeAndPop && (
          <div key="Population Structure" className="w-full rounded-lg border border-yellow-500/30 bg-yellow-500/10 text-left transition-all">
            <div className="px-3 py-2 flex items-center justify-between">
              <div className="flex items-center gap-2 overflow-hidden">
                <span className="text-yellow-600 dark:text-yellow-500 shrink-0"><Users className="w-4 h-4" /></span>
                <div className="truncate">
                  <div className="text-[10px] font-bold uppercase tracking-tight text-yellow-600/90 dark:text-yellow-500/80 truncate">Population Structure</div>
