...
export function SubmitDialog({ score, mode, roster, onClose, onSuccess }: { score: number; mode: string; roster: Partial<Record<Category, Country>>; onClose: () => void; onSuccess: () => void; }) {
  ...
  const modeLabel = mode === "daily" ? "Daily" : mode === "hard" ? "Blind" : mode === "double" ? "Double Draft" : mode === "guess" ? "Guess the Country" : "Normal"; const displayName = profile?.username ?? firebaseUser?.displayName ?? firebaseUser?.email ?? "Anonymous"; const isAuthenticated = !!firebaseUser && !!profile;
  async function handleGoogleSignIn() { setError(null); try { await signInWithGoogle(); } catch (e: any) { setError(formatAuthError(e)); } }
  async function handleEmailAuth() {
    setError(null); if (!email.trim() || !password) { setError(isSignUp ? "Enter your email and password." : "Enter your email or username and password."); return; }
